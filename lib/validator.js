...
async function loadSchema(schemaPath, schemaDir) {
  ...
  const ajv = new Ajv({
    ...
    loadSchema: async (uri) => {
      ...
      if (uri.startsWith('https://spp.dev/schemas/')) {
        const localPath = uri.replace('https://spp.dev/schemas/', '');
...
export async function validateFile(filePath, options = {}) {
  const schemaDir = options.schemaDir || path.resolve(process.cwd(), 'schemas');
  ...
  if (ext === '.json' || ext === '.jsonld') {
    fileType = ext === '.jsonld' ? 'JSON-LD' : 'JSON';
    try {
